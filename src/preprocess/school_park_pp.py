...
lat_min_10km, lat_max_10km, lon_min_10km, lon_max_10km = get_extended_lat_lng_range(f'{data_path}/train.csv', f'{data_path}/test.csv')
...
filtered_school_df = school_df[(school_df['latitude'] >= lat_min_10km) & (school_df['latitude'] <= lat_max_10km) &
                               (school_df['longitude'] >= lon_min_10km) & (school_df['longitude'] <= lon_max_10km)]

# 범위 내에 있는 공원 데이터만 필터링
filtered_park_df = park_df[(park_df['latitude'] >= lat_min_10km) & (park_df['latitude'] <= lat_max_10km) &
                           (park_df['longitude'] >= lon_min_10km) & (park_df['longitude'] <= lon_max_10km)]
