...
def calculate_total_park_area_within_radius(train_data, park_data, radius=1000):
    ...
    for idx, location in tqdm(
        unique_locations.iterrows(),
        total=unique_locations.shape[0],
        desc="Calculating total park areas",
    ):
    ...
    park_area_df = pd.DataFrame(
        total_park_areas,
        columns=["latitude", "longitude", "total_park_area_within_radius"],
    )
    return park_area_df
...
def calculate_subway_distance(train_data, subway_data):
    ...
    counts = (
        distance_df.groupby(["latitude", "longitude"])
        .agg(
